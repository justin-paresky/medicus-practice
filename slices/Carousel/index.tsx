...
export default function Carousel({ slice }: CarouselProps) {
  ...
  return (
    <section data-slice-type={slice_type} data-slice-variation={variation}>
      <ResponsiveContainer fullWidth={fullWidth} className="relative">
        <div
            ...
            <>
              {slides.map((slide, idx) => {
                ...
                return (
                  <div
                    key={uuidv4()}
                    className={`flex-grow ${isCurrentSlide ? 'bg-primary text-white' : 'my-20 hidden bg-accent text-primary md:flex'} flex flex-col justify-start gap-8 p-8`}
                  >
                    {isFilled.image(icon) && (
                      <PrismicNextImage
                        field={icon}
        ...
        <div className="absolute left-[-10px] top-24 flex w-[calc(100%_+_20px)] flex-row justify-between md:hidden">
          <button
            type="button"
            aria-label="previous"
