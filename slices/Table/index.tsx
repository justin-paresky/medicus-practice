...
const Table = ({ slice }: TableProps): JSX.Element => {
  const {
    primary: { title, description, link, label, variation, fullWidth },
    items,
    slice_type,
  ...
  return (
    <section data-slice-type={slice_type} data-slice-variation={_variation}>
      <ResponsiveContainer
        className={clsx(
          'flex flex-col gap-[25px] overflow-x-auto bg-cover bg-no-repeat px-5 md:flex-row md:gap-[50px] lg:px-24',
          css.tableContainer
        )}
        fullWidth={fullWidth}
      >
